...
void combine_images(int** rasterOut, RasterImage* image, int row, int col, int index, pthread_mutex_t lock, bool overlap);
...
std::vector<pthread_mutex_t> myWriteLock;
...
void initializeApplication(int argc, char** argv)
{
    ...
    myWriteLock.resize(numThreads);

    // initialize mutexes
    for(int i = 0; i < numThreads; i++){
        pthread_mutex_init(&myWriteLock[i], nullptr);
    }
    ...
    //create start and end row for each thread
    unsigned int m = images[0]->height / numThreads;
    unsigned int p = images[0]->height % numThreads;
    int start = 0;
    int end = m - 1;

    // fill in threadInfo structs and initialize mutexes
    for(int i = 0; i < numThreads; i++){
        if (i<p){
            end ++;
        }
        ...
        threadInfo[i].rowDist = uniform_int_distribution<unsigned int>(start, end);
        threadInfo[i].colDist = uniform_int_distribution<unsigned int>(0, images[0]->width-1);
        threadInfo[i].startRow = start;
        threadInfo[i].endRow = end;
        start = end + 1;
        end = start + m - 1;

        cout << "Thread " << i << " will focus on rows " << threadInfo[i].startRow << " to " << threadInfo[i].endRow << endl;
        cout << "Random row distribution for thread " << i << " is " << threadInfo[i].rowDist << endl;
        cout << "Random col distribution for thread " << i << " is " << threadInfo[i].colDist << endl;
...
void* threadFunc(void* argument){
    ...
    do {
        ...
        bool overlap = checkMultipleRegions(i, info->endRow, info->startRow);
        ...
        combine_images(rasterOut, info->images[image_index], i, j, info->index, info->lock, overlap);
...
void combine_images(int** rasterOut, RasterImage* image, int row, int col, int index, pthread_mutex_t lock, bool overlap){
    ...
    //lock the mutex
//    if (overlap){
//        pthread_mutex_lock(&myWriteLock[index]);
//    }
    ...
    //unlock the mutex
//    if (overlap){
//        pthread_mutex_unlock(&myWriteLock[index]);
//    }
